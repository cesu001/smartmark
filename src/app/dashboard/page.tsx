import { MOCK_NOTES, MOCK_TAGS } from "@/lib/mock-data";
...
const recentNotes = MOCK_NOTES.filter((n) => !n.isPinned).slice(0, 10);
const totalNotes = MOCK_NOTES.length;
const favoriteNotes = MOCK_NOTES.filter((n) => n.isFavorite).length;
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {pinnedNotes.map((note) => (
              <NoteCard key={note.id} note={note} tags={MOCK_TAGS} />
            ))}
          </div>
        </section>
      )}
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
          {recentCollections.map(({ borderColor, ...collection }) => (
            <CollectionCard key={collection.id} collection={collection} borderColor={borderColor} />
          ))}
        </div>
      </section>
